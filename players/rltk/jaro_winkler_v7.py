"""
v7 updates:
	tailored for player record linkage - CSV's now in different relative location
...
def get_matches(school, source):
	with open('../../teams/' + source + '/results_' + source + '.csv') as csv_file:
...
def main():
	...
	with open('../../teams/' + reference_source + '/results_' + reference_source + '.csv') as csv_file:
		csv_reader = csv.reader(csv_file)
		is_head_row = True
		for row in csv_reader:
			if is_head_row:						# skip the one head row
